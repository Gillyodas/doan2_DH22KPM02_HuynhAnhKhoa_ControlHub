...
{
    public class RegisterSupperAdminCommandValidator : AbstractValidator<RegisterSupperAdminCommand>
    {
        public RegisterSupperAdminCommandValidator()
        {
            RuleFor(x => x.Value)
            ...
            RuleFor(x => x.MasterKey)
                .NotEmpty().WithMessage(AccountErrors.MasterKeyRequired.Message);
        }
    }
}
