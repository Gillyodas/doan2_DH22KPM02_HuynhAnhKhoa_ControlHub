...
namespace ControlHub.API.Identity.Controllers
{
    ...
    public class IdentifierController : ControlHub.API.Controllers.BaseApiController
    {
        ...
        /// <summary>
        /// Add an identifier to an account
        /// </summary>
        /// <param name="command">Add identifier request</param>
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status409Conflict)]
        public async Task<IActionResult> AddIdentifier([FromBody] AddIdentifierCommand command)
        {
            _logger.LogInformation("Adding identifier to account");

            var result = await Mediator.Send(command);

            if (result.IsFailure)
            {
                _logger.LogError("Failed to add identifier: {Error}", result.Error);
                return HandleFailure(Result.Failure(result.Error));
            }

            _logger.LogInformation("Successfully added identifier");
            return Ok();
        }
